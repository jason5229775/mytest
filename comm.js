	...
	var zoom = 11;
	//var center = new T.LngLat(119.414520,28.455410);		// 初始化中心；
	var center = new EzCoord(119.414520,28.455410);		// 初始化中心点
	var center1 = new EzCoord(119.409030,28.524810);		// 打开弹窗后的中心
	var bound1 = new EzCoord(118.414760,29.014150);		// 范围点经度
	var bound2 = new EzCoord(120.213780,27.926470);
	$(function() {
		var $map = $("#map");
		if($map.length > 0) {
	...
	function initmap() {	// 初始
	map = new EzMap('map',{'mapCenter':center,'mapInitLevel':zoom,mapMaxLevel:2,mapMinLevel:18});
			...
			$.ajax({
				url : THEME_PATH + "areadata.json", dataType : "json", success : function(json) {
					...
					for(var i = 0; i < json.data.length; i++) {
							//points.push(new EzCoord(json.data[i][0], json.data[i][1]));
							points[i]=json.data[i][0]+","+json.data[i][1];
							//var pointtest = new EzCoord(119.414520,28.455410);
							
							//console.log(pointtest.coordinate_);
							Range = new Polygon(points[i], {strokeColor: "#32c58b", strokeWidth: 4, strokeOpacity: 1, fillColor: "#FFFFFF", fillOpacity: 0});
					}
					//Range = new T.Polygon(points, {strokeColor: "#32c58b", strokeWidth: 4, strokeOpacity: 1, fillColor: "#FFFFFF", fillOpacity: 0});
